...
local SERVERS, CONNS, ROOT, PLAIN = fx.SERVERS, fx.CONNS, fx.ROOT, fx.PLAIN
...
describe("resolve", function()
  it("подключение и базы по правилам", function()
    local target = setup()
    local r = target.resolve(ROOT .. "/ics_ua97/a_PRC.sql")
    eq("dgsql_dev", r.conn.name)
    eq({ "ics_ua97" }, r.dbs)
    eq("по пути ics_ua97/**", r.how)
    eq(#CONNS, #r.list)
  end)
  it("подключение выбрано руками — по правилам только базы", function()
    local target = setup()
    local r = target.resolve(ROOT .. "/icsMaster/f_PRC.sql", CONNS, conn("crocus_dev"))
    eq("crocus_dev", r.conn.name)
    eq({ "icsMaster" }, r.dbs)
  end)
  it("нет подключений", function()
    local target = setup()
    eq(
      { nil, "не найдено подключений DB_UI_* в .env проекта" },
      { target.resolve(ROOT .. "/ics_ua97/a_PRC.sql", {}) }
    )
  end)
  it("подключение не определилось", function()
    local target = setup()
    eq({ nil, "не определилось подключение" }, { target.resolve(PLAIN .. "/misc/y.sql") })
  end)
  it("база не определилась — с причиной", function()
    local target = setup()
    local r, why = target.resolve(ROOT .. "/Crocus/d_PRC.sql", CONNS, conn("dgsql_dev"))
    eq(nil, r)
    t.truthy(why:find("^не определилась база %(для crocus/%*%*"), why)
  end)
end)
